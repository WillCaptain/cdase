...
| `<app-repo>/cdase/context/` | Team runtime (roster, requirements, run_log) | **Yes** — this is the SSOT for that project |
...
| **2b** Repos found, **none** have consumer `cdase/` | `2b_none_have_cdase` | **All or none.** Ask: apply CDASE to **all** application repos (same user), or **none**? On **all** → collect user profile once, init + commit `<app>/cdase/` in **every** non-framework repo with the same roster entry. On **none** → do not init any repo. Never framework. |
...
* **All** → one profile collection, then init + commit `<repo>/cdase/` in every
  non-framework repo that needs it (same Name/UUID in each roster).
...
5. On **all**: collect profile once → for each target app repo: create `cdase/context/`,
   seed `users.context.md` with the same user, add `user.context.md` to `.gitignore`, **commit**.
6. Set `CDASE_ROOT=<app-repo>/cdase` for the repo you are working in now.
7. `check` → identity + roster for that active repo.
...
├── user_1/app-a/   ← has cdase/context/ — roster: alice
└── user_2/app-b/   ← no cdase/ — needs init
```

Agent: show alice from app-a, ask confirm or new identity. Ask: init app-b (and any other
missing repos) with that same user, or none? If all → bootstrap app-b with alice in roster.
