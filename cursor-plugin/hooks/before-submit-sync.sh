...
BOOT_OUT=$(CDASE_ROOT="$CDASE_ROOT" python3 "$CLIENT" boot 2>/dev/null || true)
if [[ -n "$BOOT_OUT" ]]; then
  BLOCKED=$(python3 - <<'PY' "$BOOT_OUT"
import json, sys
try:
    data = json.loads(sys.argv[1])
    print("1" if data.get("hub_tools_blocked") else "0")
except Exception:
    print("0")
PY
)
...
SYNC_OUT=$(CDASE_ROOT="$CDASE_ROOT" python3 "$CLIENT" sync 2>/dev/null || true)
if [[ -z "$SYNC_OUT" ]]; then
  echo '{}'
  exit 0
fi
